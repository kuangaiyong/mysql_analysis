...
class PerformanceStress:
    ...
    def generate_qps(self, queries_per_second: int = 100, duration: int = 60) -> Dict[str, int]:
        """
        ...
        queries = [
            ...
            "SELECT * FROM test_products WHERE category = %s LIMIT 10",
            "SELECT COUNT(*) as cnt FROM test_orders WHERE status = %s",
            "SELECT * FROM test_users WHERE status = %s LIMIT 100",
        ]
                    ...
                    while time.time() - start_time < duration:
                        ...
                        if "%s" in query:
                            if "id =" in query:
                                ...
                                param = random.choice(["电子产品", "服装", "食品", "家居"])
                            elif "status" in query:
                                param = random.choice(["active", "inactive", "pending", "paid", "completed"])
                            else:
                                param = None
    ...
    def generate_tps(self, transactions_per_second: int = 50, duration: int = 60) -> Dict[str, int]:
        """
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    while time.time() - start_time < duration:
                        ...
                        if operation == "insert":
                            cursor.execute(
                                "INSERT INTO test_users (name, email, status, score) VALUES (%s, %s, %s, %s)",
                                (f"user_{int(time.time()*1000)}", f"email_{random.randint(1,1000000)}@test.com", "active", random.randint(0, 100))
                            )
                            total_inserts += 1
                            ...
                            cursor.execute(
                                "DELETE FROM test_users WHERE status = %s AND id > 10000 LIMIT 1",
                                ("inactive",)
                            )
                            total_deletes += 1
                        ...
                        if total % 500 == 0:
                            print(f"  已执行 {total:,} 次事务 (I:{total_inserts} U:{total_updates} D:{total_deletes})", end="\r")
    ...
    def generate_buffer_pool_miss(self, iterations: int = 10) -> Dict[str, any]:
        """
        生成Buffer Pool未命中 - 大表扫描
        
        执行大表全扫描，导致Buffer Pool Miss增加
    ...
    def generate_concurrent_connections(
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            while time.time() - start_time < duration:
                ...
                print(f"  运行中... {elapsed:.0f}/{duration}秒, 活跃连接: {num_threads}", end="\r")
                time.sleep(1)
    ...
    def generate_temp_tables(self, iterations: int = 20) -> Dict[str, int]:
        """
        ...
        queries = [
            """
            SELECT SQL_NO_CACHE u.status, COUNT(*) as cnt, AVG(u.score) as avg_score
            FROM test_users u
            ...
            """,
            """
            SELECT SQL_NO_CACHE category, COUNT(*) as cnt, SUM(amount) as total
            FROM test_orders o
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MySQL性能监控场景脚本")
    ...
    parser.add_argument("--buffer-miss", type=int, metavar="N", help="生成Buffer Pool未命中 (扫描次数)")
    parser.add_argument("--concurrent", type=int, metavar="N", help="生成并发连接 (线程数)")
    parser.add_argument("--temp-tables", type=int, metavar="N", help="生成临时表使用 (查询次数)")
    parser.add_argument("--duration", type=int, default=30, help="持续时间（秒），默认30")
    ...
    stress = PerformanceStress(args.db)
