...
class DemoLockScenarios:
    ...
    def generate_row_lock_wait(self, duration: int = 30) -> Dict:
        """生成行锁等待 - 影响: Innodb_row_lock_waits 增加"""
        print(f"\n{'=' * 60}")
        ...
        def lock_holder():
            try:
                ...
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(
                        "UPDATE lock_test SET value = value + 1 WHERE id = %s",
                        (lock_id,),
                    )
    ...
    def generate_deadlock_scenario(self) -> Dict:
        """生成死锁场景 - 影响: Innodb_deadlocks 增加"""
        print(f"\n{'=' * 60}")
        print(f"开始生成死锁场景")
        print(f"{'=' * 60}")

        results = {
            "thread1_victim": False,
        ...
        def thread1():
            try:
                ...
                if (
                    ...
                    or "deadlock" in error_msg.lower()
    ...
    def generate_blocking_chain(
        self, chain_length: int = 3, duration: int = 30
    ) -> Dict:
        """生成阻塞链 - T1 -> T2 -> T3"""
        print(f"\n{'=' * 60}")
            ...
            try:
                ...
                with conn.cursor() as cursor:
                    ...
                    if index > 0:
                        ...
                        time.sleep(0.3)
                        print(
                            f"  [T{index + 1}] 尝试获取 T{index} 的锁 id={prev_lock_id}..."
                        )
                        cursor.execute(
                            "UPDATE lock_test SET value = value + 1 WHERE id = %s",
                            (prev_lock_id,),
    ...
    def generate_table_lock_scenario(self, duration: int = 10) -> Dict:
        """生成表锁场景 - 使用 LOCK TABLES 显式加锁"""
        print(f"\n{'=' * 60}")
        ...
        def table_lock_waiter():
            try:
                ...
                with conn.cursor() as cursor:
                    ...
                    cursor.execute("SELECT * FROM lock_test LIMIT 1")
                    cursor.fetchall()
    ...
    def generate_multiple_deadlocks(
        self, count: int = 3, interval: float = 2.0
    ) -> Dict:
        """生成多次死锁"""
        print(f"\n{'=' * 60}")
    ...
    def run_all_scenarios(self, duration: int = 20) -> Dict:
        """运行所有锁场景"""
        print("\n" + "=" * 60)
        print("运行所有锁场景")
        print("=" * 60)
        ...
        print(
            f"  初始 Innodb_row_lock_waits: {initial_status['row_locks'].get('Innodb_row_lock_waits', 0)}"
        )
        ...
        results["blocking_chain"] = self.generate_blocking_chain(
            chain_length=3, duration=duration
        )

        print("\n--- 获取最终锁状态 ---")
        final_status = self.get_lock_status()
        print(
            f"  最终 Innodb_row_lock_waits: {final_status['row_locks'].get('Innodb_row_lock_waits', 0)}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="mysql_demo 锁场景脚本")
    parser.add_argument("--row-lock", action="store_true", help="生成行锁等待")
    parser.add_argument("--deadlock", action="store_true", help="生成死锁")
    parser.add_argument(
        "--multiple-deadlocks", type=int, metavar="N", help="生成N次死锁"
    )
    parser.add_argument(
        "--blocking-chain", type=int, metavar="N", help="生成长度为N的阻塞链"
    )
    ...
    lock = DemoLockScenarios()

    if args.status:
        status = lock.get_lock_status()
        print("\n锁状态:")
        print(
            f"  Innodb_row_lock_waits: {status['row_locks'].get('Innodb_row_lock_waits', 0)}"
        )
        print(
            f"  Innodb_row_lock_time: {status['row_locks'].get('Innodb_row_lock_time', 0)}"
        )
        print(f"  Innodb_deadlocks: {status['deadlocks']}")
        print(
            f"  Table_locks_waited: {status['table_locks'].get('Table_locks_waited', 0)}"
        )
    elif args.all:
