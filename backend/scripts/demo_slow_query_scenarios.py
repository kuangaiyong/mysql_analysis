...
class DemoSlowQueryScenarios:
    ...
    def generate_full_table_scan(self, iterations: int = 5) -> Dict:
        """生成全表扫描慢查询 - EXPLAIN特征: type=ALL"""
        print(f"\n{'=' * 60}")
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                for i in range(iterations):
                    ...
                    print(
                        f"  第 {i + 1}/{iterations} 次: 扫描完成, 返回 {len(rows)} 行, 耗时 {elapsed:.2f}秒"
                    )
    ...
    def generate_filesort_query(self, iterations: int = 5) -> Dict:
        """生成文件排序慢查询 - EXPLAIN特征: Using filesort"""
        print(f"\n{'=' * 60}")
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                for i in range(iterations):
                    ...
                    print(
                        f"  第 {i + 1}/{iterations} 次: 排序完成, 返回 {len(rows)} 行, 耗时 {elapsed:.2f}秒"
                    )
    ...
    def generate_temp_table_query(self, iterations: int = 5) -> Dict:
        """生成临时表慢查询 - EXPLAIN特征: Using temporary; Using filesort"""
        print(f"\n{'=' * 60}")
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                for i in range(iterations):
                    ...
                    print(
                        f"  第 {i + 1}/{iterations} 次: 聚合完成, 返回 {len(rows)} 行, 耗时 {elapsed:.2f}秒"
                    )
    ...
    def generate_subquery_scenarios(self, iterations: int = 5) -> Dict:
        """生成子查询慢查询 - EXPLAIN特征: DEPENDENT SUBQUERY"""
        print(f"\n{'=' * 60}")
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                for i in range(iterations):
                    ...
                    print(
                        f"  第 {i + 1}/{iterations} 次: 子查询完成, 返回 {len(rows)} 行, 耗时 {elapsed:.2f}秒"
                    )
    ...
    def generate_like_pattern_scenarios(self, iterations: int = 5) -> Dict:
        """生成LIKE前缀通配符慢查询 - EXPLAIN特征: type=ALL (无法使用索引)"""
        print(f"\n{'=' * 60}")
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                for i in range(iterations):
                    ...
                    print(
                        f"  第 {i + 1}/{iterations} 次: LIKE '{pattern}' 完成, 返回 {len(rows)} 行, 耗时 {elapsed:.2f}秒"
                    )
    ...
    def generate_join_without_index(self, iterations: int = 5) -> Dict:
        """生成无索引JOIN慢查询 - EXPLAIN特征: Using join buffer"""
        print(f"\n{'=' * 60}")
                ...
                for i in range(iterations):
                    ...
                    cursor.execute(
                        ...
                        SELECT SQL_NO_CACHE 
                            ...
                            n.value
                        FROM users u
                        JOIN no_index_data n ON u.username = n.name
                        WHERE u.status = %s
                        LIMIT 500
                    """,
                        (random.choice(["active", "inactive", "banned"]),),
                    ...
                    print(
                        f"  第 {i + 1}/{iterations} 次: JOIN完成, 返回 {len(rows)} 行, 耗时 {elapsed:.2f}秒"
                    )
    ...
    def generate_complex_query(self, iterations: int = 3) -> Dict:
        """生成复杂组合慢查询 - 多种特征: 全表扫描、文件排序、临时表、子查询"""
        print(f"\n{'=' * 60}")
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                for i in range(iterations):
                    ...
                    print(
                        f"  第 {i + 1}/{iterations} 次: 复杂查询完成, 返回 {len(rows)} 行, 耗时 {elapsed:.2f}秒"
                    )
        ...
        print(f"  平均耗时: {avg_time:.2f}秒")
        print(
            f"  EXPLAIN特征: type=ALL, Using filesort, Using temporary, DEPENDENT SUBQUERY"
        )
        print(f"{'=' * 60}")

        return {"iterations": iterations, "avg_time": avg_time, "results": results}

    def run_all_scenarios(self, iterations: int = 5) -> Dict:
        """运行所有慢查询场景"""
        print("\n" + "=" * 60)
        print("运行所有慢查询场景")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="mysql_demo 慢查询场景脚本")
    parser.add_argument("--all", action="store_true", help="运行所有慢查询场景")
    parser.add_argument(
        "--full-scan", type=int, metavar="N", help="生成全表扫描 (次数)"
    )
    ...
    parser.add_argument(
        "--like-pattern", type=int, metavar="N", help="生成LIKE通配符 (次数)"
    )
    parser.add_argument(
        "--join-no-index", type=int, metavar="N", help="生成无索引JOIN (次数)"
    )
    ...
    scenarios = DemoSlowQueryScenarios()
